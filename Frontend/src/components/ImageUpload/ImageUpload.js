...
const ImageUpload = ({ 
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    if (validFiles.length === 0) {
      alert('Please select valid image files (JPEG, PNG, WebP) under 5MB each.');
      return;
    }
    ...
    try {
      const response = await axios.post('/api/upload/multiple', formData, {
        ...
        onUploadProgress: (progressEvent) => {
          ...
          setUploadProgress(prev => ({ ...prev, uploading: percentCompleted }));
        }
      });

      if (response.data.success) {
        const newImageUrls = response.data.data.images.map(img => img.imageUrl);
  ...
  return (
    <div className={`image-upload-container ${className}`}>
      <div
        className={`image-upload-dropzone ${dragActive ? 'drag-active' : ''} ${disabled ? 'disabled' : ''}`}
